...
cloud モードでは `verifyMcpServiceCaller` を追加：`Authorization` に Cloud Run **身份 JWT**（`run.invoker` 権限を持つ呼び出し元）が必要。委譲ユーザートークンは `x-user-access-token` で別送。

### 4.5 トークン委譲（Token Delegation）

トークン委譲は、ユーザーの OAuth アクセストークンをサービス境界を越えて渡し、下流が各ホップでユーザー再認証なしに **本人確認** できるようにする。Cloud Run プライベートサービスは `Authorization` に別途 **身份 JWT** が必要なため、**デュアルヘッダ** を使用：

| ホップ                                    | `Authorization`          | ユーザー委譲ヘッダ                                    |
| ----------------------------------------- | ------------------------ | ----------------------------------------------------- |
...
| remote-agent → bq-mcp-server（Cloud Run） | サービス身份 JWT         | `x-user-access-token: <user_access_token>`            |
| IDE → gcloud プロキシ → MCP               | プロキシが SA JWT に置換 | ユーザートークンは `x-session-authorization` に保持   |

**原則：** Cloud Run IAM は _サービスに到達できる主体_ を証明；委譲ユーザートークンは _どの Google ユーザーに代わって_ 実行するかを証明。ユーザー OAuth トークンは **BigQuery API に直接送信しない**。
...
| 3    | Remote + Direct tool       | **Prove identity** クリック                                                   | `"credential_source": "user_oauth_access_token"` とメールを含む JSON |
...
| Web remote + direct 本人確認     | JSON `user_oauth_access_token` | ユーザートークンが bq-mcp-server に到達 |
