...
- Demo and deploy tooling must be portable across any GCP project; do not hardcode project IDs in scripts, docs, or config.
- Keep web-chat demo UX simple: default to agent chat for Send; expose deterministic auth proof as explicit actions, not modes users must learn.
...
- `remote-agent` hosts multiple A2A agents on one Cloud Run service (`/agents/{id}/agent-card.json`, RFC 9727 `/.well-known/api-catalog`); runtime enable/disable via `GET/PATCH /agent-policy` (in-memory, resets on restart). `@agent-platform/a2a-server` shares mount/policy helpers used by `remote-agent`. Legacy BigQuery card at `/.well-known/agent-card.json` requires path-specific mounts—a catch-all A2A middleware blocks other agent routes when BigQuery is disabled.
- `remote-agent` also exposes MCP (`/mcp` with a `chat` tool); `bq-mcp` exposes direct BigQuery MCP tools (`list_datasets`, `get_authenticated_user`). Chain: web-chat → remote-agent → bq-mcp (web-chat never calls bq-mcp directly). On the agent hop, remote-agent mints a short-lived delegation JWT (`x-delegation-token`; shared `DELEGATION_JWT_SECRET` / `.delegation-jwt-secret` from deploy scripts); bq-mcp verifies the JWT instead of the raw Google token. IDE proxy and other direct-to-MCP paths still pass the Google access token.
- `web-chat` discovers agents from the API catalog, selects an agent in the UI, and chats via A2A. Chat mode (local vs remote) is stored in an httpOnly cookie via `/api/chat-mode`, not the request body. Intent-based routing sends BigQuery-flavored prompts to the bigquery agent when enabled even if General is selected. Auth proof: **Send** uses `/api/chat` (agent LLM path); **Prove identity** / **List datasets** use `/api/chat-proof` (direct MCP JSON path) — no user-facing `demo_mode` toggle. Auth probe presets (`full`, `no_iam`, `iam_only`, `no_session`) control session/IAM/A2A/MCP layers—remote chat requires `full`; proof presets use `/api/chat-proof`. When `.delegation-jwt-secret` exists, `run-web-chat.sh` sets `DELEGATION_JWT_EXCHANGE=true` for auth-trace hop-exchange display. Playbook: `docs/auth-proof.md`. Standalone proof reports: `docs/report.md`, `docs/report.ja.md`.
...
- Root `postinstall` builds `mcp-auth`, `agent-client`, `a2a-server`, and `agent-cli`; `pretest` runs full `pnpm build` so CI ESLint and vitest resolve workspace packages on clean runners.
